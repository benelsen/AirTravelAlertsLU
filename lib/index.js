...
import {
  compose, prop, map, curry, assoc, eqProps, propEq, propSatisfies, over, lensProp, union,
  replace, not, flip, contains, merge, toUpper, adjust, join, props, ifElse, split, filter,
} from 'ramda'
...
const createEventType = data => {
  ...
  if ( (Math.abs(timeDiff) >= 15 && data.type === 'arrival') && (Math.abs(timeDiff) >= 30 && data.type === 'departure') ) {
