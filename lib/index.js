...
import {Pushover} from 'winston-pushover'
...
import {twitterCredentials, fetchInterval, pushover} from '../data/config.json'
...
const createEventType = data => {
  ...
  if ( (gteAbs(timeDiff, 15) && isArrival(data)) || (gteAbs(timeDiff, 30) && isDeparture(data)) ) {
    const status = `${data.changes ? 'initial' : 'change'}_${timeDiff > 0 ? 'delayed' : 'early'}_${data.type}`
